...
# Custom CSS for Professional Dark-Mode Trading Terminal UI
st.markdown("""
    <style>
    ...
    div.stMetric, div.css-1r6slb0, div[data-testid="stVerticalBlock"] > div > div.stChatMessage {
        background-color: #111827;
...
def get_historical_baseline(symbol: str, interval: str):
    try:
        period_map = {"15m": "59d", "1h": "730d", "4h": "730d", "1d": "1y"}
        fetch_period = period_map.get(interval, "1y")
        fetch_interval = "1h" if interval == "4h" else interval

        df_hist = _fetch_raw_history(symbol, fetch_period, fetch_interval)
        if df_hist is None or df_hist.empty or not all(c in df_hist.columns for c in ['Close', 'High', 'Low']):
...
def run_monte_carlo_fast(S0: float, mu: float, sigma: float, steps: int, n_sims: int = 1000, model_type: str = "Standard (Normal Distribution)", log_returns=None, jump_params=None):
    ...
    if model_type == "Standard (Normal Distribution)":
        ...
        daily_returns = (mu - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * shocks
        ...
        daily_returns = (mu - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * t_shocks
        ...
        daily_returns = drift + sigma * np.sqrt(dt) * shocks + jump_factor
    else:
        shocks = np.random.normal(0, 1, size=(steps - 1, n_sims))
        daily_returns = (mu - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * shocks
...
if page == "📖 Beginner's Guide & Single Simulator":
    ...
    st.markdown("##### Quantitative risk estimation and dynamic MFE/MAE trade targeting.")
    ...
    user_ticker_input = st.sidebar.text_input("Asset Ticker Symbol", value="EURUSD")
    ...
    jump_params = (4.0 / 252.0, -0.05, 0.05) if model_type == "Jump-Diffusion (Merton)" else None
    ...
    if close_data is None or log_returns is None or log_returns.empty:
        st.error(f"❌ Could not load or parse valid price data for **{ticker}** on interval **{candle_interval}**.")
        st.stop()
    ...
    def render_live_simulator_dashboard():
        ...
        p25, p75, p95, p5 = np.percentile(sim_matrix, 25, axis=1), np.percentile(sim_matrix, 75, axis=1), np.percentile(sim_matrix, 95, axis=1), np.percentile(sim_matrix, 5, axis=1)
        ...
        col1.metric("Live Spot Price", f"{S0:,.{decimals}f}")
        col2.metric("Optimized Limit Entry", f"{optimal_entry:,.{decimals}f}")
        col3.metric("Optimized Target (TP1)", f"{tp_1:,.{decimals}f}", delta=f"+{((tp_1/optimal_entry)-1)*100:.2f}%")
        col4.metric("Optimized Stop Loss", f"{stop_loss:,.{decimals}f}", delta=f"{((stop_loss/optimal_entry)-1)*100:.2f}%", delta_color="inverse")
        ...
        with col_right:
            ...
            if rr_ratio >= 1.5:
                ...
                st.warning("🟡 **Acceptable Setup**")
            else:
                st.error("🔴 **Low R:R Ratio**")

        st.markdown("---")
        st.subheader(f"📊 Projected Path Cone & Optimal Levels ({forecast_steps} Steps)")
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color='rgba(255,255,255,0)'), name='90% Simulation Band'
        ))
        fig.add_trace(go.Scatter(
            x=np.concatenate([steps_axis, steps_axis[::-1]]),
            y=np.concatenate([p75, p25[::-1]]),
            fill='toself', fillcolor='rgba(56, 189, 248, 0.18)',
            line=dict(color='rgba(255,255,255,0)'), name='50% Simulation Band'
        ))
        fig.add_trace(go.Scatter(
            x=steps_axis, y=np.median(sim_matrix, axis=1), mode='lines',
            name='Median Path', line=dict(color="#facc15", width=2.5)
        ))

        fig.add_hline(y=optimal_entry, line_dash="dash", line_color="#38bdf8", annotation_text="Optimized Entry")
        fig.add_hline(y=tp_1, line_dash="dash", line_color="#34d399", annotation_text="TP 1")
        fig.add_hline(y=stop_loss, line_dash="dash", line_color="#f87171", annotation_text="MAE Stop Loss")
...
# =========================================================
    ...
    st.title("📊 Multi-Asset Summary Dashboard")
    st.markdown("##### Concurrent MFE/MAE risk and target analysis across a multi-asset watchlist.")
    ...
    if st.button("🚀 Run Comparative MFE/MAE Analysis") or "summary_df" not in st.session_state:
        ...
        for res in data_results:
            ...
            for steps in selected_steps_list:
                ...
                exp_return_pct = ((float(np.median(final_prices)) / S0) - 1) * 100
                decimals = 4 if S0 < 10 else 2

                summary_rows.append({
                    "Ticker": t_symbol, "Interval": selected_interval, "Live Price": round(S0, decimals),
                    "Steps": steps, "Limit Entry": round(optimal_entry, decimals), "Stop Loss": round(stop_loss, decimals),
                    "Take Profit 1": round(tp_1, decimals), "R:R Ratio": round(rr_ratio, 2), "Exp Return (%)": round(exp_return_pct, 2)
                })
    ...
    if summary_df.empty:
        st.error("No valid data could be retrieved for the specified tickers.")
        st.stop()
    ...
    st.download_button("📥 Download Matrix as CSV", data=csv_data, file_name="gipsy_trading_matrix.csv", mime="text/csv")
